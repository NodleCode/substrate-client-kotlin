...
import io.nodle.substratesdk.account.Mnemonic
import io.nodle.substratesdk.account.toSS58
...
class TestMnemonic {
    ...
    @Test
    fun stage1_generateFromSubstrateTestVector() {
        for (vector in substrateVectors) {
            val seed = Mnemonic.seedFromMnemonic(vector[0].split(" "))
            assertThat(seed.toHex(), CoreMatchers.equalTo(vector[1]))

            val key = Mnemonic.generateEd25519FromMnemonic(vector[0].split(" "), "Substrate")
            val privateKey = key.encoded.toHex()
            assertThat(privateKey, CoreMatchers.equalTo(vector[2].substring(0, 64)))
        }
    }

    @Test
    fun stage2_generateNewMnemonic() {
        val twelve1 = Mnemonic.newTwelveWords()
        assertThat(twelve1.size, CoreMatchers.equalTo(12))

        val twelve2 = Mnemonic.newTwelveWords()
        assertThat(twelve2.size, CoreMatchers.equalTo(12))

        assertThat(twelve1 == twelve2, CoreMatchers.equalTo(false))
    }
}